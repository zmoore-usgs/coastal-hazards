 ...
 * 
 */
@DescribeProcess(title = "CchImport", description = "Imports a feature collection into the catalog")
public class CchImportProcess implements GSProcess {

	static final Logger LOGGER = Logging.getLogger(CchImportProcess.class);

	private Catalog catalog;
	private GeoServerDataDirectory dataDir;
	
	public CchImportProcess(Catalog catalog) {
		this.catalog = catalog;
	}

	public CchImportProcess(Catalog catalog, GeoServerDataDirectory dataDirectory) throws IOException {
		this.catalog = catalog;
	...
	public String execute(
		...
		// Check if the data already exists in the store location - if it does,
		// it needs to be deleted before I can try re-importing. Otherwise, the
		// process will append to the shapefile which can give us very unexpected results
		if (storeInfo.getType().toLowerCase(Locale.getDefault()).contains("shapefile")) {
			File fileLocation = new File(((String)storeInfo.getConnectionParameters().get("url")).replace("file:", ""));
			if (fileLocation.exists()) {
				FilenameFilter filenameFilter = FileFilterUtils.prefixFileFilter(name);
				File[] existingShapefiles = fileLocation.listFiles(filenameFilter);
				for (File existingShapefile : existingShapefiles) {
					if (existingShapefile.delete()) {
						LOGGER.log(Level.FINE, "Deleted {}", existingShapefile.getCanonicalPath());
					} else {
						LOGGER.log(Level.WARNING, "Could not delete {} -- Shapefile import may not succeed or imported shapefile may be incorrect", existingShapefile.getCanonicalPath());
					}
				}
			}
		}
