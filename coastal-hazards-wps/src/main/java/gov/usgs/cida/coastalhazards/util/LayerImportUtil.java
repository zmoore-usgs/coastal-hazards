...
public class LayerImportUtil {
    ...
    public void checkIfLayerExists(String workspace, String layerName) {
        LayerInfo layerByName = catalog.getLayerByName(workspace + ":" + layerName);
        if (layerByName != null) {
            throw new LayerAlreadyExistsException("Cannot overwrite layer");
        }
    }
    
    /**
     * Does not support styles at this point
     * @param collection
     * @param workspace
     * @param store
     * @param layerName
