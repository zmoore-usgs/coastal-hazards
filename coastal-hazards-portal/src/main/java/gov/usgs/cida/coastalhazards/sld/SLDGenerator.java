...
public class SLDGenerator {
	...
	public static SLDGenerator getGenerator(Item item, Item selectedItem, Integer ribbon, boolean filterVisible) {
        ...
        if (itemType == Item.ItemType.data) {
            ...
            if (null != conf) {
                generator = new SLDGenerator(item, selectedItem, ribbon, conf, filterVisible);
            }
	 ...
	 * @param config 
	 */
	protected SLDGenerator(Item item, Item selectedItem, Integer ribbon, SLDConfig config, boolean filterVisible) {
		this.item = item;
		this.ribbon = ribbon;
        if (config instanceof ShorelineConfig) {
            ShorelineConfig slc = (ShorelineConfig)config;
            slc.finalize(selectedItem, filterVisible);
            this.config = slc;
        } else {
            this.config = config;
        }
